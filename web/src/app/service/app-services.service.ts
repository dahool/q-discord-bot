...
export class AppService extends BaseService implements IAppService {
  ...
  getEvents(id: string): Observable<Schedule[]> {
    return this.executeGet('events/' + id).pipe(map(v => v.map((vin: any) => parseDate(vin, 'dtStart', 'dtEnd'))))
  }
  ...
  listZones(): Observable<Territory[]> {
    return this.executeGet('zones').pipe(map(v => v.map((vin: any) => parseDate(vin, 'next'))))
  }
